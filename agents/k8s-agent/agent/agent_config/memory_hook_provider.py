...
class MemoryHookProvider(HookProvider):
    ...
    def seed_memory(self, actor_id: str):
        """Seed memory with initial EKS cluster and contact information"""
        try:
            ...
            # Seed with EKS cluster and contact information
            cluster_interactions = [
                ("EKS cluster sample-eks-cluster contact is platform-team@company.com", "USER"),
                ("I've recorded EKS cluster sample-eks-cluster with contact platform-team@company.com.", "ASSISTANT"),
            ]
            ...
            try:
                self.client.create_event(
                    ...
                    messages=cluster_interactions
                )
                logger.info("Seeded EKS cluster contact information")
            except Exception as e:
                logger.warning(f"Error seeding cluster data: {e}")
        ...
        try:
            ...
            if len(messages) >= 2 and messages[-1]["role"] == "assistant":
                ...
                if user_query and agent_response:
                    ...
                    logger.info("Saved K8s diagnostics interaction to memory")

        except Exception as e:
            logger.error(f"Failed to save K8s diagnostics interaction: {e}")
    ...
    def register_hooks(self, registry: HookRegistry) -> None:
        ...
        logger.info("K8s diagnostics memory hooks registered")
