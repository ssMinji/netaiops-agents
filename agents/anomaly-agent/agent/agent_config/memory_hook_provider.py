            ...
            if messages[-1]["role"] in ("user", "assistant"):
                ...
                if messages[-1]["role"] == "user":
                    self._add_context_user_query(
                        namespace=f"anomaly/{self.actor_id}/context",
                        query=messages[-1]["content"][0]["text"],
                        init_content="These are anomaly detection contexts:", event=event)
                    self._add_context_user_query(
                        namespace=f"anomaly/{self.actor_id}/history",
                        query=messages[-1]["content"][0]["text"],
                        init_content="These are past anomaly detection records:", event=event)
