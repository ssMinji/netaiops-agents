...
class MemoryHookProvider(HookProvider):
    """Hook provider for automatic memory management - Network diagnostics"""
    ...
    def seed_memory(self, actor_id: str):
        """Seed memory with initial network context information"""
        try:
            try:
                if self.namespaces:
            ...
            # Seed with AWS network context
            network_interactions = [
                ("AWS network infrastructure in ap-northeast-2 region. VPCs, Transit Gateway, Route 53 DNS, and NAT Gateways are in use. Contact: platform-team@company.com", "USER"),
                ("AWS 네트워크 정보를 기록했습니다. ap-northeast-2 리전에서 VPC, Transit Gateway, Route 53 DNS, NAT Gateway를 사용 중이며, 담당자는 platform-team@company.com 입니다.", "ASSISTANT"),
            ]
            ...
            try:
                self.client.create_event(
                    ...
                    messages=network_interactions
                )
                logger.info("Seeded AWS network context information")
            except Exception as e:
                logger.warning(f"Error seeding network data: {e}")
        ...
        try:
            ...
            if len(messages) >= 2 and messages[-1]["role"] == "assistant":
                ...
                if user_query and agent_response:
                    ...
                    logger.info("Saved network diagnostics interaction to memory")

        except Exception as e:
            logger.error(f"Failed to save network diagnostics interaction: {e}")
    ...
    def register_hooks(self, registry: HookRegistry) -> None:
        ...
        logger.info("Network diagnostics memory hooks registered")
