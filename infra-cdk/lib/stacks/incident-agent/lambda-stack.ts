...
export interface IncidentAgentLambdaStackProps {
  lambdaRole: iam.IRole;
}

export class IncidentAgentLambdaStack extends Construct {
  ...
  constructor(scope: Construct, id: string, props: IncidentAgentLambdaStackProps) {
    super(scope, id);

    const cfg = CONFIG.incidentAgent;
    const lambdaSrcBase = path.join(__dirname, '..', '..', '..', 'lambda-src', 'incident-agent');
    ...
    // 4. Chaos Lambda — UI에서 직접 호출하는 용도 (MCP Gateway에 연결하지 않음)
    ...
    // 6. GitHub Lambda — MCP Gateway에 연결하여 Agent가 직접 호출
