...
export interface IncidentAgentGatewayStackProps {
  auth: CognitoAuth;
  executionRoleArn: string;
  lambdas: IncidentAgentLambdaStack;
}

/**
 * Incident Agent Gateway - Lambda targets for Datadog, OpenSearch, ContainerInsight tools.
 *
 * Each Lambda target includes inline tool schemas so the Gateway can route
 * tool calls to the correct Lambda function.
 */
export class IncidentAgentGatewayStack extends Construct {
  public readonly gateway: McpGateway;

  constructor(scope: Construct, id: string, props: IncidentAgentGatewayStackProps) {
    super(scope, id);
