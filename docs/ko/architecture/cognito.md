...
- **Auth method**: 브라우저 로그인 → 콜백 URL이 토큰 수신
- **목적**: 직접 에이전트 호출 테스트를 위한 테스트/개발 환경(`test_agent.py`)
...
- `test_agent.py`는 사용자 인증을 위해 Web Client ID + PKCE 사용
- 브라우저 로그인 후, 콜백 코드를 토큰으로 교환
- UI 백엔드 없이 AgentCore Runtime을 직접 호출
...
- **Agent Pool** (`K8sAgentPool`): 스코프 `gateway:read`, `gateway:write`, `invoke`
  ...
  - Web Client: `K8sWebClient` — `test_agent.py`를 통한 PKCE 테스트
...
- **Auth Pool** (`IncidentAnalysisPool`): 스코프 `invoke`
  ...
  - Web Client: `IncidentAnalysisWebClient` — 테스트
...
| `{prefix}/web_client_id` | Web Client ID (존재하는 경우) |
